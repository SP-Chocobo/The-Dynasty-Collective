...
def main():
    ...
    roster_ids = [str(i) for i in range(12, 0, -1)]   # REVERSED seat order (arm B)
    order = dstrat.generate_pick_order(roster_ids, 26, "snake")
    print("picks:", len(order), flush=True)

    traj = ds.simulate_full_draft(
        merger, players_db, LEAGUE, order, config_label="FOURTH_AND_FOREVER_REVSEAT",
    ...
    json.dump({"league": "Fourth and Forever", "picks": picks,
              ...
              open(OUT / "ff_draft_revseat.json", "w"), indent=1)
