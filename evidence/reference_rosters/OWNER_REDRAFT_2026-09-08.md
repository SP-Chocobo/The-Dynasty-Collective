...
## IDENTIFIED, 2026-09-14 — this league has a name, and it is FFCL Group A

Appended six days after the fact. Nothing above is edited; this block only adds what was
missing.

**This document describes FFCL Group A.** It was recorded here as "the owner's redraft
league" and carried through `#216` under the anonymous label `OWNER_3RR_SF_noTE`. On
2026-09-14 the owner supplied the league by name, with screenshots, and every format field
matches this document exactly:

| field | this document (09-08) | `data/league_captures/ffcl_group_a.json` (09-14) |
|---|---|---|
| starters | QB / 2 WR / 2 RB / 2 W-R-T flex / 1 W-R flex / 1 SUPER_FLEX | `QB RB RB WR WR FLEX FLEX WRRB_FLEX SUPER_FLEX` |
| bench | 5 | 5 |
| teams | 12 | 12 |
| `rec` | PPR | 1.0 |
| `bonus_rec_te` | 0.5 TE premium | 0.5 |
| horizon | REDRAFT | redraft |
| `draft_type` | 3RR | `3rr` |
| trading | NO TRADING | `allowed: false` |

Zero conflicts across eight fields.

**What the 09-14 capture adds that was genuinely not here:** the name; the full 42-value
scoring rulebook (this document carried two of the forty-two — `rec` and the TE premium);
**2 IR slots**, which this document does not mention at all (it says "9 starters, 5 bench,
14 total" — the real roster is 16 slots); the six-pod / 72-manager structure that is the
stated *reason* for the trading ban; and the draft board.

**What it does not add:** the format, the turn structure, the no-trading consequence chain,
or the missing-inputs table. Those were all here first.

### The "OUT-OF-SAMPLE" claim in the header is now stale

It was true on 09-08. `#216` subsequently built this league into
`run_216_bench_probe.py:280` as `OWNER_LEAGUE` / `OWNER_3RR_SF_noTE`, and that arm really
does draft third-round reversal — `build_league` returns `OWNER_LEAGUE["draft_type"]`
(`"3rr"`) where every other branch returns `"snake"`. The label is honest. So this format is
IN sample for the `#216` flex-share and shared-slot instruments, and out of sample for the
34-arm battery, which still runs plain snake everywhere.
